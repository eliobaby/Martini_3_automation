...
def main():
    ...
    csv_path          = os.path.join(cancer_dir, "Targeted_Therapies_Small_Drug_Molecule.csv")
    working_dir       = os.path.join(cancer_dir, "Working")
    half_dir          = os.path.join(cancer_dir, "Half_working")
    fake_dir          = os.path.join(cancer_dir, "Fake_working")
    large_dir         = os.path.join(cancer_dir, "Working_Large")
    non_working_path  = os.path.join(cancer_dir, "non-working.txt")
    summary_path      = os.path.join(cancer_dir, "summary.txt")
    errors_path       = os.path.join(cancer_dir, "errors.txt")
    ...
    with open(csv_path, newline='') as f:
        ...
        for row in reader:
            ...
            comp_id, smiles = row[0], row[2]
            print(f"Attempt ID {comp_id}...", flush=True)

            try:
                # 1) run main_batch.py in Working/
                ...
                # 6) Read .gro to classify large (only if not fake or half)
                ...
                # clean up partial folder if any
